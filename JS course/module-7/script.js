...
const getUsersByFriend = (users, name) => {
  const filteredUsers = users.filter(user => user.friends.includes(name))
  .map(user => user.name);
  return filteredUsers;
};
...
const getUniqueSkills = users => {
  const skillsArr = users.reduce((allSkills, user) => {
   ...
   skillsArr.forEach(el => {
     if (!uniqueSkills.includes(el)) uniqueSkills.push(el)
   })
  return uniqueSkills;
};
...
const getNamesSortedByFriendsCount = users => {
  const sortedUsers = users.sort((user1, user2) => user1.friends.length - user2.friends.length);
  const NamesSortedByFriendsCount = sortedUsers.map(user => user.name)
